...
from corvid.model.multimodal_encoder.s2wrapper import forward as multiscale_forward
...
class SiglipVisionTower(nn.Module):
    ...
    def feature_select(self, image_forward_outs):
        # len 25, each is (batch, 577, 1024) - (batch, 729, 1152)
        image_features = image_forward_outs.hidden_states[self.select_layer]
        if self.select_feature == 'patch':
            # not remove the first token --> (batch, 576, 1024) - (batch, 729, 1152)
    ...
    @torch.no_grad()
    def extract_bbox_region_features(self, images, bboxes):
        image_features = self.forward(images)
        region_features = []
        for image_feature, bbox in zip(image_features, bboxes):
            region_feature = self.extract_bbox_region_feature(image_feature, bbox)
    ...
    def dummy_feature(self):
        return torch.zeros(
            1, self.config.vision_config.hidden_size,
...
class SiglipVisionTowerS2(SiglipVisionTower):
    ...
    def load_model(self, device_map=None):
        ...
        self.image_processor = CustomSiglipImageProcessor.from_pretrained(
            self.vision_tower_name)
        self.vision_tower = SiglipVisionModel.from_pretrained(
            self.vision_tower_name, device_map=device_map)
        self.vision_tower.requires_grad_(False)
