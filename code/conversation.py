        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            for i, (role, message) in enumerate(messages):
                if i == 0:
                    ...
                    assert role == self.roles[
                        0], "first message should come from user"
                if message:
                    if type(message) is tuple:
                        message, _, _ = message
                    if i == 0:
                ...
                if type(msg) is tuple:
                    ...
                    for img in image:
                        if self.is_image_file(img):
                            img_b64_str = self.process_image(img, "Default",
                                                             return_pil=False,
                                                             image_format="JPEG")
    ...
    def dict(self):
        if len(self.get_images()) > 0:
            return {
                "system": self.system,
                "roles": self.roles,
                "messages": [[x, y[0] if type(y) is tuple else y] for x, y in
                             self.messages],
                "offset": self.offset,
                "sep": self.sep,
                "sep2": self.sep2,
...
conv_gemma_instruct = Conversation(system="", roles=(
    "<start_of_turn>user\n", "<start_of_turn>model\n"), version="gemma",
                                   messages=[], offset=0,
...
conv_llava_llama_3 = Conversation(
    ...
    offset=0,
    sep_style=SeparatorStyle.LLAMA_3,
    # tokenizer=AutoTokenizer.from_pretrained("playground/model/Meta-Llama-3-8B-Instruct"),
    stop_token_ids=[128009],
)
...
conv_llava_llama_3_1 = Conversation(
    ...
    tokenizer=AutoTokenizer.from_pretrained("playground/model/meta-llama/Meta-Llama-3.1-8B-Instruct"),
...
if __name__ == "__main__":
    ...
    print(conv_llava_llama_3_1.get_prompt())
